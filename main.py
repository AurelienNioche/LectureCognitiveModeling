...
def run_simulation(agent_model, param, seed=None):
...
def plot_ax_custom(ax, y_label, title=None, legend=True):
...
def scatter_binary_choices(ax, y,
                           colors=None,
                           scatter_labels=None,
    ...
    plot_ax_custom(ax, y_label=y_label, title=title)
...
def behavior_basic_plot(choices, successes):

    n_rows = 2
    fig, axes = plt.subplots(nrows=n_rows, figsize=(4, 2.5*n_rows))

    scatter_binary_choices(ax=axes[0],
                           y=choices,
                           title='Choices')

    scatter_binary_choices(ax=axes[1],
...
behavior_basic_plot(choices=CHOICES, successes=SUCCESSES)


def behavior_average_plot(choices, successes, axes=None):
    ...
    ax = axes[0]
    scatter_binary_choices(ax=ax, y=choices, title="Choices")
    ax.legend()
    ...
    plot_ax_custom(ax=ax, y_label="Freq. choice", title="Choices (freq.)")

    ax = axes[2]
    scatter_binary_choices(ax=ax, y=np.asarray(successes, dtype=int),
                           y_label="success",
                           scatter_labels=('failure', 'success'),
                           colors=np.array(['red', 'green']),
                           title="Successes")
    ...
    plot_ax_custom(ax=ax, y_label="success (average)",
...
behavior_average_plot(choices=CHOICES, successes=SUCCESSES)
...
def latent_variable_plot(q_values, p_choices, choices, successes,
                         axes=None):
    ...
    plot_ax_custom(ax=ax, y_label="value", title="Q-values", legend=False)
    ...
    plot_ax_custom(ax=ax, y_label="value",
...
latent_variable_plot(q_values=Q_VALUES,
                     p_choices=P_CHOICES,
                     successes=SUCCESSES,
                     choices=CHOICES)
...
def population_simulation(model, param, n=30):
    ...
    for i in range(n):
        ...
        choices, successes \
            = run_simulation(agent_model=model, param=param)
    ...
    return pop_choices, pop_q_values, pop_p_choices, pop_successes
...
def pop_latent_variable_plot(
    ...
    plot_ax_custom(ax=ax, y_label="value", title="Q-values")
    ...
    plot_ax_custom(ax=ax, y_label='p', title='Probabilities')
    ...
    plot_ax_custom(ax=ax, y_label='freq. choice', title="Choices")

    ax = axes[3]

    plot_mean_std(ax=ax, y=np.asarray(successes, dtype=int))
    plot_ax_custom(ax=ax, y_label='success', title="Successes", legend=False)
...
def pop_analysis():
    pop_choices, pop_q_values, pop_p_choices, pop_successes = \
